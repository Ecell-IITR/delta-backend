...
from users.models import Student, Person, Company
...
class AvatarUploadAPI(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if profile_image:
            ...
            return Response(
                {"error_message": "Profile image not found"},
...
class SelfProfile(generics.RetrieveUpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if user.student_profile:
            ...
            if social_links:
                for social_link in social_links:
                    website = get_object_or_404(
                        Website, name=social_link.get("website_name") or None
                    )
                    profile_url = social_link.get("profile_url") or None
                    if profile_url:
                        if user_profile.social_links.filter(
                            profile_url=profile_url, website=website
                        ).exists():
                            return Response(
                                {
                                    "error_message": "Already profile added for %s"
                                    % (website.name)
                                },
                                status=status.HTTP_400_BAD_REQUEST,
                            )

                        user_profile.social_links.create(
                            website=website, profile_url=profile_url
                        )
                    else:
                        return Response(
                            {
                                "error_message": "Profile url not found for %s!"
                                % (website.name)
                            },
                            status=status.HTTP_400_BAD_REQUEST,
                        )
            ...
            return Response(
                StudentSerializer(user_profile).data, status=status.HTTP_200_OK
            )
    ...
    def post(request, redirect_uri_type=None, *args, **kwargs):
        ...
        if code:
            ...
            response_data = requests.post(
                url=token_url, data=encoded_data, headers=headers
            )
            if "json" in response_data.headers.get("Content-Type"):
                ...
                if access_token:
                    ...
                    fetch_user_headers = {"Authorization": "Bearer %s" % access_token}
                    get_user_data = requests.get(
                        url=user_data_url, headers=fetch_user_headers
                    )
                    if "json" in get_user_data.headers.get("Content-Type"):
                        ...
                        contact_information = (
                            get_user_body.get("contactInformation") or None
                        )
                        person_info = get_user_body.get("person") or None
                        biological_information = (
                            get_user_body.get("biologicalInformation") or None
                        )
                        ...
                        if username:
                            ...
                            if user is None:
                                user = Person.objects.create(
                                    ...
                                    is_channeli_oauth=True,
                                )
                                profile_image = (
                                    person_info.get("displayPicture") or None
                                )
                                ...
                                phone_number = (
                                    contact_information.get("primaryPhoneNumber")
                                    or None
                                )
                                ...
                                date_of_birth = (
                                    biological_information.get("dateOfBirth") or None
                                )
                            ...
                            return Response(
                                {"token": token.key, "role": user.role_type},
                ...
                return Response(
                    {"error_message": "Invalid code type"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        else:
            return Response(
                {"error_message": "OAuth code is missing!"},
                status=status.HTTP_400_BAD_REQUEST,
            )
