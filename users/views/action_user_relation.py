...
class ActionRelationBaseView(APIView):
    def initial(self, request, *args, **kwargs):
        ...
        try:
            action_on_person = get_object_or_404(Person, username=action_on_person_username)
        except:
...
class ActionView(ActionRelationBaseView, APIView):
    ...
    def post(request, *args, **kwargs):
        ...
        if check_action_user_relation.exists():
            return Response({"error_message": 'Relation already exists!'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            return Response({'error_message': 'Username or action key invalid!'}, status=status.HTTP_400_BAD_REQUEST)
