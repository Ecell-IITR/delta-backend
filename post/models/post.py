...
class AbstractPost(TimestampedModel):
    ...
    user = models.ForeignKey(
        Person, related_name="%(app_label)s_%(class)s", on_delete=models.CASCADE
    )
    ...
    post_expiry_date = models.DateTimeField(blank=True, verbose_name="Post expiry date")
    ...
    location = models.ForeignKey(
        to=Location, blank=True, null=True, on_delete=models.CASCADE
    )

    bookmarks = models.ManyToManyField(
        to="users.Student", related_name="%(app_label)s_%(class)s_bookmarks", blank=True
    )

    tags = models.ManyToManyField(
        Tag, related_name="%(app_label)s_%(class)s_tags", blank=True
    )
